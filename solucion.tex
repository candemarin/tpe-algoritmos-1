...
(\forall i : \mathbb{Z}) (0 \leq i < |j| \implicaLuego \text{posValida}(j[i][0]) \yLuego (\text{minasAdyacentes}(t, j[i][0]) = j[i][1])
}
...
\begin{proc}{PlantarBanderita}{\In n : {$\mathbb{Z}$}, \In J : \jugadas, \In P : \pos, \Inout b : \banderitas }{}
    ...
    %intento pedir que los dos primeros (son la posicion) de cada tripla de j al multiplicarlos sean menores que n, para decir que estan en el tablero
    %
    
    \pred{NValido}{\In n : {$\mathbb{Z}$}} {(( 1 < n) \y ( \neg esPrimo(n)) }
    
    %pido que n sea mayor que uno y no sea primo para que el tablero me quede rectangular y no hayan tableros de 1 sola linea por la cantidad de celdas  pero capaz es sobreespecificar
    %
    
    \pred{PosicionValida}{\In n : {$\mathbb{Z}$}, \In P : \pos} {( 0 \leq J[i]_{0} * J[i]_{1} \leq n ) \y ( 0 \leq J[i]_{0} , J[i]_{1} ) }
    
    % lo mismo , que la multiplicacion me de menor que n para decir que estan en rango del tablero
    %
